...
export default function SignInForm() {
	...
	useEffect(() => {
		...
		if (errorParam === "CredentialsSignin") {
			...
			setError(
				"Google sign-in failed. Check: (1) NEXTAUTH_URL=http://localhost:3000 in .env.local, (2) Authorized redirect URI in Google Cloud Console is exactly http://localhost:3000/api/auth/callback/google, (3) Terminal where npm run dev is running for the actual error."
			);
		}
	}, [searchParams]);
			...
			<div className="w-[50%] bg-white flex flex-col justify-center py-16 relative">
				...
				<form onSubmit={onSubmit} className="space-y-6 flex-1 flex flex-col ml-16 mr-16">
					<button
						...
						disabled={loading}
						className="w-full flex items-center justify-center gap-2 border-2 py-3 rounded-lg font-semibold text-base transition-colors"
